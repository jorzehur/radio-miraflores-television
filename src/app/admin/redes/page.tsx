...
interface Red { id: string; platform: string; url: string; embedUrl: string | null; username: string; followers: string; active: boolean; sortOrder: number }
...
export default function AdminRedes() {
  ...
  const [form, setForm] = useState({ platform: 'facebook', url: '', embedUrl: '', username: '', followers: '', sortOrder: 0 })
  ...
  async function handleCreate() {
    ...
    setForm({ platform: 'facebook', url: '', embedUrl: '', username: '', followers: '', sortOrder: 0 }); setShowForm(false); loadItems()
  }

  async function handleUpdate(id: string, data: Partial<Red>) {
          ...
          <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
            <div className="flex items-start gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleUpdate(item.id, { platform: item.platform, url: item.url, embedUrl: item.embedUrl || null, username: item.username, followers: item.followers, sortOrder: item.sortOrder, active: item.active })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">URL</label><input type="text" value={item.url} onChange={e => setItems(prev => prev.map(it => it.id === item.id ? { ...it, url: e.target.value } : it))} className="w-full px-4 py-2.5 border rounded-lg outline-none" placeholder="https://facebook.com/..." /></div>
