...
export default function RankingSection() {
  const [rankingData, setRankingData] = useState<WPRankingItem[]>([])
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    async function fetchRanking() {
      try {
        const res = await fetch('/api/wordpress/ranking')
        const data = await res.json()
        if (data.status === 'ok' && data.data.length > 0) {
          setRankingData(data.data.slice(0, 4))
        }
      } catch {
        // Use fallback data
      } finally {
  ...
  const displayData = rankingData.length > 0 ? rankingData : fallbackRanking.map((item, i) => ({
    ...item,
    ...cardStyles[i],
  }))
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/10 rounded-full mb-4">
            ...
            <span className="text-white/90 text-sm font-medium">
              {isLoading ? 'Cargando...' : 'Lo más escuchado'}
            </span>
          </div>
          ...
          {rankingData.length > 0 && (
            <p className="text-white/40 text-xs mt-2">Datos desde WordPress</p>
          )}
        </motion.div>
        ...
        >
          {displayData.slice(0, 4).map((item, index) => {
            ...
            const weeks = item.weeks || 0
            const trend = item.trend || 'same'
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-gray-500 font-medium">{weeks} sem.</span>
                  </div>
                </div>
                ...
                <p className="text-gray-600 text-sm font-medium mb-1">{artist}</p>
